...
type WelfareReport = {
  ...
  recommendations: Array<{
    ...
    applicationLocation: string; deadline: string; risks: string[]; url: string;
  }>;
};
  ...
  return (
    <main className="app-shell">
      ...
      <section className="paid-report-card featured">
        <div className="report-card-copy"><span className="report-launch-badge">놓치면 아까운 숨은 혜택</span><h2>내 조건에 맞는 지원금,<br />얼마나 놓치고 있을까요?</h2><p>가구·소득·주거·직업을 분석해 나에게 맞는 혜택과 신청 방법을 한눈에 정리해 드려요.</p></div>
        <button className="report-price-button report-price-button-large" onClick={() => { track("welfare_report_entry_click", { source: "hero" }); setReport(storedReport?.report || null); setReportOpen(true); }}><small>{storedReport ? "지난 분석 결과" : "AI 숨은 혜택 분석 · 1회"}</small><span>{storedReport ? "내 맞춤 결과 다시 보기" : "내 숨은 복지 혜택 확인하기"}</span>{!storedReport && <em>990원</em>}<b>›</b></button>
      </section>
      ...
      {reportOpen && (
        <div className="modal-backdrop" role="presentation" onClick={() => !reportLoading && setReportOpen(false)}>
          <section className="detail-sheet report-sheet" role="dialog" aria-modal="true" aria-label="맞춤 복지 리포트" onClick={(event) => event.stopPropagation()}>
            ...
            <span className="detail-agency">AI 맞춤 복지 카드 · 1회 이용권</span>
            ...
            {report ? (
              <div className="report-result">
                ...
                <h4 className="report-policy-heading">내게 맞는 복지 카드 {report.recommendations.length}개</h4>
                {report.recommendations.map((item, index) => <article className="report-policy" key={`${item.policyId}-${index}`}>
                  ...
                  <ReportMini icon="₩" title="받을 수 있는 혜택" text={item.benefitEstimate} />
                  <ReportList icon="✓" title="자격 확인" items={item.eligibilityChecks} />
                  <ReportMini icon="⌖" title="신청 장소·위치" text={item.applicationLocation} />
                  <ReportList icon="▤" title="필요서류" items={item.requiredDocuments} />
                  <ReportList icon="→" title="신청 순서" items={item.applicationSteps} ordered />
                  <div className="report-card-meta"><ReportMini icon="◷" title="신청기한" text={item.deadline} /><ReportMini icon="!" title="주의사항" text={item.risks.join(" · ")} warning /></div>
                  <button onClick={() => setExternalTarget(item.url)}>신청 페이지·공식 안내 열기 <b>›</b></button>
                </article>)}
              ...
              <>
                ...
                <label className="report-field"><span>현재 상황을 자세히 알려주세요 <small>선택</small></span><textarea maxLength={700} value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} placeholder="예: 경기도에서 혼자 월세로 살고 있는 취업준비생이에요. 생활비와 취업교육비가 부담되고, 신청할 수 있는 청년 지원과 준비서류를 알고 싶어요." /><small>토스에서 불러온 나이·지역은 다시 적지 않아도 돼요. 소득 변화, 부채, 실직, 돌봄 등 특별한 상황을 적으면 더 정확해져요. · {reportDetails.length}/700</small></label>
...
function FirstVisitOnboarding({ step, total, profile, tossProfile, consentedData, authLoading, authError, setProfile, onLogin, onUserData, onStep, onDone, onSkip }: { step: number; total: number; profile: ReportProfile; tossProfile: TossProfile | null; consentedData: ConsentedData | null; authLoading: boolean; authError: string; setProfile: Dispatch<SetStateAction<ReportProfile>>; onLogin: () => void; onUserData: () => void; onStep: (step: number) => void; onDone: () => void; onSkip: () => void }) {
  ...
  return <div className="first-onboarding" role="dialog" aria-modal="true" aria-label="맞춤 혜택 설정">
    ...
    <section className="onboarding-content">
      {step === 0 ? <div className="onboarding-brand-intro"><img src={BRAND_ICON} alt="나라가쏜다 로고" /><strong>나라가쏜다</strong><span className="onboarding-benefit-count">정부 혜택 {total > 0 ? total.toLocaleString() : "20,000+"}개 통합</span><h1>나도 몰랐던<br /><em>정부 복지혜택</em>을 찾아보세요</h1><p>토스로 로그인하면 내 조건에 맞는 혜택을 빠르게 찾아드려요.</p></div> : <><span className="onboarding-eyebrow">내 조건에 맞는 숨은 혜택 찾기</span><h1>{steps[step].title}</h1><p>{steps[step].description}</p></>}
      {step === 0 && <div className="onboarding-auth-card login-only"><button disabled={authLoading || Boolean(tossProfile)} onClick={onLogin}>{tossProfile ? `${tossProfile.name || "토스 사용자"}님 로그인 완료` : authLoading ? "연결 중…" : "토스로 로그인하기"}</button><small>로그인 정보는 암호화해 전송하며, AI에 이름·전화번호·이메일을 보내지 않아요.</small></div>}
...
function ReportMini({ icon, title, text, warning = false }: { icon: string; title: string; text: string; warning?: boolean }) {
  return <div className={warning ? "report-mini warning" : "report-mini"}><b><i>{icon}</i>{title}</b><p>{text}</p></div>;
}

function ReportList({ icon, title, items, ordered = false, warning = false }: { icon: string; title: string; items: string[]; ordered?: boolean; warning?: boolean }) {
  const children = items.map((item) => <li key={item}>{item}</li>);
  return <div className={warning ? "report-mini warning" : "report-mini"}><b><i>{icon}</i>{title}</b>{ordered ? <ol>{children}</ol> : <ul>{children}</ul>}</div>;
}
