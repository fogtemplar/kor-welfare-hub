...
export async function fetchGovServicePolicies(opts: { forceRefresh?: boolean } = {}): Promise<Policy[]> {
  if (!opts.forceRefresh) {
    ...
    if (cached && !process.env.GOV_SERVICE_API_KEY?.trim() && !process.env.BOKJIRO_API_KEY?.trim()) return cached.items;
  }

  const key = process.env.GOV_SERVICE_API_KEY?.trim() || process.env.BOKJIRO_API_KEY?.trim();
  if (!key) return [];
